...
from sistema_bancario.utils import (
    ClientesCsv,
    ContasCsv,
    TransacaoCsv,
...
class Historico:
    ...
    def transacoes_do_dia(self) -> List[Extrato]:
        return [transacao for transacao in self.extrato if transacao["data"].date() == datetime.now().date()]
...
class ContaCorrente(Conta):
    ...
    def sacar(self, valor: Decimal) -> bool:
        ...
        qtd_saques_dia = len(
            [
                transacao
                for transacao in self.historico.extrato
                if transacao["valor"].is_signed() and transacao["data"].date() == datetime.now().date()
            ]
        )
...
def carrega_clientes(
    ...
    for cliente in clientes:
        ...
        for conta in contas:
            ...
            for transacao in transacoes:
                if (
                    transacao["cliente_id"] != conta.cliente.id
...
class SistemaBancario:
    def __init__(self) -> None:
        print(f"Iniciando Sistema Bancario v{sistema_bancario.version}")
